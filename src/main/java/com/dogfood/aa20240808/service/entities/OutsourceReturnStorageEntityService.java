...
import java.time.ZonedDateTime;
/**
* auto generate OutsourceReturnStorageEntityService ServiceImpl
*
* @author sys
*/
@Service
public class OutsourceReturnStorageEntityService extends AbstractService {
    @Resource
    private OutsourceReturnStorageEntityMapper mapper;
    @Resource
    private RelationInnerService relationInnerService;
    @Resource
    private OutsourceReturnStorageEntityService entityService;
    ...
    public OutsourceReturnStorageEntityService() {
        entityFieldTitleNameMap.put("委外加工入库退回单号", "returnCode");
        entityFiledColumnNameMap.put("returnCode", "return_code");
    ...
    **/
    public List<OutsourceReturnStorageEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<OutsourceReturnStorageEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, OutsourceReturnStorageEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + OutsourceReturnStorageEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
         ...
         * auto gen get method
         **/
        public OutsourceReturnStorageEntity get( String returnCode ) { 
            if ( returnCode == null ) { 
                throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
            }

            OutsourceReturnStorageEntity entity = mapper.selectOne( returnCode ); 
    ...
    **/
    ...
    public OutsourceReturnStorageEntity create(OutsourceReturnStorageEntity entity) {
        if (null == entity) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<OutsourceReturnStorageEntity> batchCreate(List<OutsourceReturnStorageEntity> entities) {
        ...
        // fill default value
        List<OutsourceReturnStorageEntity> batchList = new ArrayList<>(100);
        for (OutsourceReturnStorageEntity entity : entities) {
            entity.setCreatedBy(StringUtils.isBlank(entity.getCreatedBy()) ? currentUserName : entity.getCreatedBy());
            entity.setUpdatedBy(StringUtils.isBlank(entity.getUpdatedBy()) ? currentUserName : entity.getUpdatedBy());
            paramValidate(entity);
    ...
    public void beforeUpdate(OutsourceReturnStorageEntity entity) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "OutsourceReturnStorageEntity");
        }
        if ( entity.getReturnCode() == null ) { 
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
        }
    }
    ...
    **/
    @Transactional
    public OutsourceReturnStorageEntity update(OutsourceReturnStorageEntity entity, List<String> updateFields) {
        beforeUpdate(entity);

        // updateFields为null时，默认全量更新
        if (null != updateFields && updateFields.size() == 1 &&  updateFields.contains("returnCode")) {
            return entity;
        } else {
            UserContext.UserInfo currentUserInfo = UserContext.getCurrentUserInfo();
            String currentUserName = null == currentUserInfo ? null : currentUserInfo.getUserName();
            ...
            return get(entity.getReturnCode());
        }
    }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<OutsourceReturnStorageEntity> batchUpdate(List<OutsourceReturnStorageEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }

        if (updateFields != null && updateFields.size() == 1 && updateFields.contains("returnCode")) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long delete( String returnCode ) { 
        if ( returnCode == null ) { 
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
        }
        OutsourceReturnStorageEntity entity = mapper.selectOne( returnCode ); 
        ...
        return mapper.delete( returnCode ); 
    }
    ...
    **/
    ...
    public long batchDelete(List<String> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("OutsourceReturnStorage", "outsource_return_storage", "returnCode", "return_code"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<OutsourceReturnStorageEntity>  entities = mapper.selectList(inQueryFilter);
        for (OutsourceReturnStorageEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<OutsourceReturnStorageEntity> data = ExcelUtil.read(file.getInputStream(), type, OutsourceReturnStorageEntity.class, entityFieldTitleNameMap);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public OutsourceReturnStorageEntity createOrUpdate(OutsourceReturnStorageEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, OutsourceReturnStorageEntity.class);
        }
        ...
        if ( entity.getReturnCode() == null ) { 
            ...
            OutsourceReturnStorageEntity existEntity = mapper.selectOne(entity.getReturnCode()); 
            if (null == existEntity) {
                // insert
                UserContext.UserInfo currentUserInfo = UserContext.getCurrentUserInfo();
                String currentUserName = null == currentUserInfo ? null : currentUserInfo.getUserName();
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(OutsourceReturnStorageEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
