...
import java.time.ZonedDateTime;
/**
* auto generate ProductInStorageEntityService ServiceImpl
*
* @author sys
*/
@Service
public class ProductInStorageEntityService extends AbstractService {
    @Resource
    private ProductInStorageEntityMapper mapper;
    @Resource
    private RelationInnerService relationInnerService;
    @Resource
    private ProductInStorageEntityService entityService;
    ...
    public ProductInStorageEntityService() {
        entityFieldTitleNameMap.put("产品入库单号", "num");
        entityFiledColumnNameMap.put("num", "num");
        ...
        entityFieldTitleNameMap.put("生产车间(部门)", "departmentId");
        entityFiledColumnNameMap.put("departmentId", "department_id");
    ...
    **/
    public List<ProductInStorageEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<ProductInStorageEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, ProductInStorageEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + ProductInStorageEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
         ...
         * auto gen get method
         **/
        public ProductInStorageEntity get( String num ) { 
            if ( num == null ) { 
                throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
            }

            ProductInStorageEntity entity = mapper.selectOne( num ); 
    ...
    **/
    ...
    public ProductInStorageEntity create(ProductInStorageEntity entity) {
        if (null == entity) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<ProductInStorageEntity> batchCreate(List<ProductInStorageEntity> entities) {
        ...
        // fill default value
        List<ProductInStorageEntity> batchList = new ArrayList<>(100);
        for (ProductInStorageEntity entity : entities) {
            entity.setCreatedBy(StringUtils.isBlank(entity.getCreatedBy()) ? currentUserName : entity.getCreatedBy());
            entity.setUpdatedBy(StringUtils.isBlank(entity.getUpdatedBy()) ? currentUserName : entity.getUpdatedBy());
            paramValidate(entity);
    ...
    public void beforeUpdate(ProductInStorageEntity entity) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "ProductInStorageEntity");
        }
    ...
    **/
    @Transactional
    public ProductInStorageEntity update(ProductInStorageEntity entity, List<String> updateFields) {
        beforeUpdate(entity);

        // updateFields为null时，默认全量更新
        if (null != updateFields && updateFields.size() == 1 &&  updateFields.contains("num")) {
            return entity;
        } else {
            UserContext.UserInfo currentUserInfo = UserContext.getCurrentUserInfo();
            String currentUserName = null == currentUserInfo ? null : currentUserInfo.getUserName();
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<ProductInStorageEntity> batchUpdate(List<ProductInStorageEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }

        if (updateFields != null && updateFields.size() == 1 && updateFields.contains("num")) {
    ...
    **/
    ...
    public long batchDelete(List<String> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("ProductInStorage", "product_in_storage", "num", "num"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<ProductInStorageEntity>  entities = mapper.selectList(inQueryFilter);
        for (ProductInStorageEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<ProductInStorageEntity> data = ExcelUtil.read(file.getInputStream(), type, ProductInStorageEntity.class, entityFieldTitleNameMap);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public ProductInStorageEntity createOrUpdate(ProductInStorageEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, ProductInStorageEntity.class);
        }
        ...
        if ( entity.getNum() == null ) { 
            ...
            ProductInStorageEntity existEntity = mapper.selectOne(entity.getNum()); 
            if (null == existEntity) {
                // insert
                UserContext.UserInfo currentUserInfo = UserContext.getCurrentUserInfo();
                String currentUserName = null == currentUserInfo ? null : currentUserInfo.getUserName();
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(ProductInStorageEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
