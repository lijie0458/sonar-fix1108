...
import com.dogfood.aa20240808.repository.entities.PermissionGroupEntityMapper;
...
import java.time.ZonedDateTime;
/**
* auto generate PermissionGroupEntityService ServiceImpl
*
* @author sys
*/
@Service
public class PermissionGroupEntityService extends AbstractService {
    @Resource
    private PermissionGroupEntityMapper mapper;
    @Resource
    private RelationInnerService relationInnerService;
    @Resource
    private PermissionGroupEntityService entityService;
    ...
    public PermissionGroupEntityService() {
        ...
        entityFieldTitleNameMap.put("权限组名称", "name");
    ...
    **/
    public List<PermissionGroupEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<PermissionGroupEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, PermissionGroupEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + PermissionGroupEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
         ...
         * auto gen get method
         **/
        public PermissionGroupEntity get( Long id ) { 
            if ( id == null ) { 
                throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
            }

            PermissionGroupEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public PermissionGroupEntity create(PermissionGroupEntity entity) {
        if (null == entity) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<PermissionGroupEntity> batchCreate(List<PermissionGroupEntity> entities) {
        ...
        // fill default value
        List<PermissionGroupEntity> batchList = new ArrayList<>(100);
    ...
    public void beforeUpdate(PermissionGroupEntity entity) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "PermissionGroupEntity");
        }
    ...
    **/
    @Transactional
    public PermissionGroupEntity update(PermissionGroupEntity entity, List<String> updateFields) {
        beforeUpdate(entity);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<PermissionGroupEntity> batchUpdate(List<PermissionGroupEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("PermissionGroup", "permission_group", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<PermissionGroupEntity>  entities = mapper.selectList(inQueryFilter);
        for (PermissionGroupEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<PermissionGroupEntity> data = ExcelUtil.read(file.getInputStream(), type, PermissionGroupEntity.class, entityFieldTitleNameMap);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public PermissionGroupEntity createOrUpdate(PermissionGroupEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, PermissionGroupEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            PermissionGroupEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                UserContext.UserInfo currentUserInfo = UserContext.getCurrentUserInfo();
                String currentUserName = null == currentUserInfo ? null : currentUserInfo.getUserName();
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(PermissionGroupEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
