...
import com.dogfood.aa20240808.repository.entities.OrganisationEntityMapper;
...
import java.time.ZonedDateTime;
/**
* auto generate OrganisationEntityService ServiceImpl
*
* @author sys
*/
@Service
public class OrganisationEntityService extends AbstractService {
    @Resource
    private OrganisationEntityMapper mapper;
    @Resource
    private RelationInnerService relationInnerService;
    @Resource
    private OrganisationEntityService entityService;
    ...
    public OrganisationEntityService() {
        entityFieldTitleNameMap.put("组织id", "id");
        ...
        entityFieldTitleNameMap.put("组织名称", "name");
        entityFiledColumnNameMap.put("name", "name");
        entityFieldTitleNameMap.put("父级组织id", "parentId");
        entityFiledColumnNameMap.put("parentId", "parent_id");
        entityFieldTitleNameMap.put("是否有子组织", "hasChildren");
        entityFiledColumnNameMap.put("hasChildren", "has_children");
    ...
    **/
    public List<OrganisationEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<OrganisationEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, OrganisationEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + OrganisationEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
         ...
         * auto gen get method
         **/
        public OrganisationEntity get( Long id ) { 
            if ( id == null ) { 
                throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
            }

            OrganisationEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public OrganisationEntity create(OrganisationEntity entity) {
        ...
        entity.setParentId(getOrDefault(entity.getParentId(), 0L));
        entity.setHasChildren(getOrDefault(entity.getHasChildren(), false));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<OrganisationEntity> batchCreate(List<OrganisationEntity> entities) {
        ...
        // fill default value
        List<OrganisationEntity> batchList = new ArrayList<>(100);
        for (OrganisationEntity entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            entity.setCreatedBy(StringUtils.isBlank(entity.getCreatedBy()) ? currentUserName : entity.getCreatedBy());
            entity.setUpdatedBy(StringUtils.isBlank(entity.getUpdatedBy()) ? currentUserName : entity.getUpdatedBy());
            entity.setParentId(getOrDefault(entity.getParentId(), 0L));
    ...
    public void beforeUpdate(OrganisationEntity entity) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "OrganisationEntity");
        }
    ...
    **/
    @Transactional
    public OrganisationEntity update(OrganisationEntity entity, List<String> updateFields) {
        beforeUpdate(entity);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<OrganisationEntity> batchUpdate(List<OrganisationEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("Organisation", "Organisation", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<OrganisationEntity>  entities = mapper.selectList(inQueryFilter);
        for (OrganisationEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<OrganisationEntity> data = ExcelUtil.read(file.getInputStream(), type, OrganisationEntity.class, entityFieldTitleNameMap);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public OrganisationEntity createOrUpdate(OrganisationEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, OrganisationEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            OrganisationEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                UserContext.UserInfo currentUserInfo = UserContext.getCurrentUserInfo();
                String currentUserName = null == currentUserInfo ? null : currentUserInfo.getUserName();
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(OrganisationEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
