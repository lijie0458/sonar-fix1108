...
import com.dogfood.aa20240808.repository.entities.ModuleTableEntityMapper;
...
import java.time.ZonedDateTime;
/**
* auto generate ModuleTableEntityService ServiceImpl
*
* @author sys
*/
@Service
public class ModuleTableEntityService extends AbstractService {
    @Resource
    private ModuleTableEntityMapper mapper;
    @Resource
    private RelationInnerService relationInnerService;
    @Resource
    private ModuleTableEntityService entityService;
    ...
    public ModuleTableEntityService() {
        entityFieldTitleNameMap.put("模块id", "id");
        ...
        entityFieldTitleNameMap.put("模块名称", "name");
    ...
    **/
    public List<ModuleTableEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<ModuleTableEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, ModuleTableEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + ModuleTableEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
         ...
         * auto gen get method
         **/
        public ModuleTableEntity get( Long id ) { 
            if ( id == null ) { 
                throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
            }

            ModuleTableEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public ModuleTableEntity create(ModuleTableEntity entity) {
        if (null == entity) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<ModuleTableEntity> batchCreate(List<ModuleTableEntity> entities) {
        ...
        // fill default value
        List<ModuleTableEntity> batchList = new ArrayList<>(100);
        for (ModuleTableEntity entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            entity.setCreatedBy(StringUtils.isBlank(entity.getCreatedBy()) ? currentUserName : entity.getCreatedBy());
            entity.setUpdatedBy(StringUtils.isBlank(entity.getUpdatedBy()) ? currentUserName : entity.getUpdatedBy());
            paramValidate(entity);
    ...
    public void beforeUpdate(ModuleTableEntity entity) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "ModuleTableEntity");
        }
    ...
    **/
    @Transactional
    public ModuleTableEntity update(ModuleTableEntity entity, List<String> updateFields) {
        beforeUpdate(entity);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<ModuleTableEntity> batchUpdate(List<ModuleTableEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("ModuleTable", "ModuleTable", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<ModuleTableEntity>  entities = mapper.selectList(inQueryFilter);
        for (ModuleTableEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<ModuleTableEntity> data = ExcelUtil.read(file.getInputStream(), type, ModuleTableEntity.class, entityFieldTitleNameMap);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public ModuleTableEntity createOrUpdate(ModuleTableEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, ModuleTableEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            ModuleTableEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                UserContext.UserInfo currentUserInfo = UserContext.getCurrentUserInfo();
                String currentUserName = null == currentUserInfo ? null : currentUserInfo.getUserName();
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(ModuleTableEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
