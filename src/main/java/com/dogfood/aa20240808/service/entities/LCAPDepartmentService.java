...
import com.dogfood.aa20240808.repository.entities.LCAPDepartmentMapper;
...
import java.time.ZonedDateTime;
/**
* auto generate LCAPDepartmentService ServiceImpl
*
* @author sys
*/
@Service
public class LCAPDepartmentService extends AbstractService {
    @Resource
    private LCAPDepartmentMapper mapper;
    @Resource
    private RelationInnerService relationInnerService;
    @Resource
    private LCAPDepartmentService entityService;
    ...
    public LCAPDepartmentService() {
        ...
        entityFieldTitleNameMap.put("部门名称", "name");
        entityFiledColumnNameMap.put("name", "name");
        entityFieldTitleNameMap.put("部门标识", "deptId");
        entityFiledColumnNameMap.put("deptId", "dept_id");
        entityFieldTitleNameMap.put("父部门标识", "parentDeptId");
        entityFiledColumnNameMap.put("parentDeptId", "parent_dept_id");
    ...
    **/
    public List<LCAPDepartment> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<LCAPDepartment> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, LCAPDepartment.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + LCAPDepartment.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
    ...
    **/
    ...
    public LCAPDepartment create(LCAPDepartment entity) {
        if (null == entity) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<LCAPDepartment> batchCreate(List<LCAPDepartment> entities) {
        ...
        // fill default value
        List<LCAPDepartment> batchList = new ArrayList<>(100);
        for (LCAPDepartment entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            entity.setCreatedBy(StringUtils.isBlank(entity.getCreatedBy()) ? currentUserName : entity.getCreatedBy());
            entity.setUpdatedBy(StringUtils.isBlank(entity.getUpdatedBy()) ? currentUserName : entity.getUpdatedBy());
            paramValidate(entity);
    ...
    public void beforeUpdate(LCAPDepartment entity) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "LCAPDepartment");
        }
    ...
    **/
    @Transactional
    public LCAPDepartment update(LCAPDepartment entity, List<String> updateFields) {
        beforeUpdate(entity);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<LCAPDepartment> batchUpdate(List<LCAPDepartment> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("LCAPDepartment", "lcap_department_a6d592", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<LCAPDepartment>  entities = mapper.selectList(inQueryFilter);
        for (LCAPDepartment entity : entities) {
            relationInnerService.onDelete(entity);
        }
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<LCAPDepartment> data = ExcelUtil.read(file.getInputStream(), type, LCAPDepartment.class, entityFieldTitleNameMap);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public LCAPDepartment createOrUpdate(LCAPDepartment entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, LCAPDepartment.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            LCAPDepartment existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                UserContext.UserInfo currentUserInfo = UserContext.getCurrentUserInfo();
                String currentUserName = null == currentUserInfo ? null : currentUserInfo.getUserName();
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(LCAPDepartment entity, List<String> updateFields, AbstractQueryFilter filter) {
