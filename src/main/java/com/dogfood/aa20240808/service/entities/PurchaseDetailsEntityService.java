...
import java.time.ZonedDateTime;
/**
* auto generate PurchaseDetailsEntityService ServiceImpl
*
* @author sys
*/
@Service
public class PurchaseDetailsEntityService extends AbstractService {
    @Resource
    private PurchaseDetailsEntityMapper mapper;
    @Resource
    private RelationInnerService relationInnerService;
    @Resource
    private PurchaseDetailsEntityService entityService;
    ...
    public PurchaseDetailsEntityService() {
        ...
        entityFieldTitleNameMap.put("申请数量", "request_quantity");
        entityFiledColumnNameMap.put("request_quantity", "request_quantity");
        entityFieldTitleNameMap.put("期望到货日期", "expected_delivery_date");
        entityFiledColumnNameMap.put("expected_delivery_date", "expected_delivery_date");
        entityFieldTitleNameMap.put("关联申请单号", "related_request_number");
        entityFiledColumnNameMap.put("related_request_number", "related_request_number");
        entityFieldTitleNameMap.put("物料编码", "material_code");
        entityFiledColumnNameMap.put("material_code", "material_code");
        entityFieldTitleNameMap.put("已下推数量", "down_quantity");
        entityFiledColumnNameMap.put("down_quantity", "down_quantity");
        entityFieldTitleNameMap.put("本次下推数量", "this_quantity");
        entityFiledColumnNameMap.put("this_quantity", "this_quantity");
        entityFieldTitleNameMap.put("下单状态", "orderStatus");
        entityFiledColumnNameMap.put("orderStatus", "order_status");
    ...
    **/
    public List<PurchaseDetailsEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<PurchaseDetailsEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, PurchaseDetailsEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + PurchaseDetailsEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
         ...
         * auto gen get method
         **/
        public PurchaseDetailsEntity get( Long id ) { 
            if ( id == null ) { 
                throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
            }

            PurchaseDetailsEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public PurchaseDetailsEntity create(PurchaseDetailsEntity entity) {
        if (null == entity) {
        ...
        entity.setRequest_quantity(getOrDefault(entity.getRequest_quantity(), BigDecimal.ZERO));
        entity.setExpected_delivery_date(getOrDefault(entity.getExpected_delivery_date(), LocalDate.now()));
        entity.setDown_quantity(getOrDefault(entity.getDown_quantity(), new BigDecimal("0")));
        entity.setOrderStatus(getOrDefault(entity.getOrderStatus(), OrderStatusEnumEnum.FIELD_2));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<PurchaseDetailsEntity> batchCreate(List<PurchaseDetailsEntity> entities) {
        ...
        // fill default value
        List<PurchaseDetailsEntity> batchList = new ArrayList<>(100);
        for (PurchaseDetailsEntity entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            entity.setCreatedBy(StringUtils.isBlank(entity.getCreatedBy()) ? currentUserName : entity.getCreatedBy());
            entity.setUpdatedBy(StringUtils.isBlank(entity.getUpdatedBy()) ? currentUserName : entity.getUpdatedBy());
            entity.setRequest_quantity(getOrDefault(entity.getRequest_quantity(), BigDecimal.ZERO));
    ...
    public void beforeUpdate(PurchaseDetailsEntity entity) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "PurchaseDetailsEntity");
        }
    ...
    **/
    @Transactional
    public PurchaseDetailsEntity update(PurchaseDetailsEntity entity, List<String> updateFields) {
        beforeUpdate(entity);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<PurchaseDetailsEntity> batchUpdate(List<PurchaseDetailsEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("PurchaseDetails", "purchase_details", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<PurchaseDetailsEntity>  entities = mapper.selectList(inQueryFilter);
        for (PurchaseDetailsEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<PurchaseDetailsEntity> data = ExcelUtil.read(file.getInputStream(), type, PurchaseDetailsEntity.class, entityFieldTitleNameMap);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public PurchaseDetailsEntity createOrUpdate(PurchaseDetailsEntity entity, List<String> updateFields) {
        if (null == entity) {
        ...
        if ( entity.getId() == null ) { 
            ...
            PurchaseDetailsEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                UserContext.UserInfo currentUserInfo = UserContext.getCurrentUserInfo();
                String currentUserName = null == currentUserInfo ? null : currentUserInfo.getUserName();
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(PurchaseDetailsEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
