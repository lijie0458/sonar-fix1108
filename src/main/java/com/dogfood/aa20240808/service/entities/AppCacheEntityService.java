...
import com.dogfood.aa20240808.repository.entities.AppCacheEntityMapper;
...
import java.time.ZonedDateTime;
/**
* auto generate AppCacheEntityService ServiceImpl
*
* @author sys
*/
@Service
public class AppCacheEntityService extends AbstractService {
    @Resource
    private AppCacheEntityMapper mapper;
    @Resource
    private RelationInnerService relationInnerService;
    @Resource
    private AppCacheEntityService entityService;
    ...
    public AppCacheEntityService() {
        ...
        entityFieldTitleNameMap.put("健", "key");
        entityFiledColumnNameMap.put("key", "key");
        entityFieldTitleNameMap.put("值", "value");
        entityFiledColumnNameMap.put("value", "value");
        entityFieldTitleNameMap.put("过期时间（秒）", "expiration");
        entityFiledColumnNameMap.put("expiration", "expiration");
    ...
    **/
    public List<AppCacheEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<AppCacheEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, AppCacheEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + AppCacheEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
        ...
        public AppCacheEntity get( Long id ) { 
            ...
            AppCacheEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public AppCacheEntity create(AppCacheEntity entity) {
        if (null == entity) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<AppCacheEntity> batchCreate(List<AppCacheEntity> entities) {
        ...
        // fill default value
        List<AppCacheEntity> batchList = new ArrayList<>(100);
        for (AppCacheEntity entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            entity.setCreatedBy(StringUtils.isBlank(entity.getCreatedBy()) ? currentUserName : entity.getCreatedBy());
            entity.setUpdatedBy(StringUtils.isBlank(entity.getUpdatedBy()) ? currentUserName : entity.getUpdatedBy());
            paramValidate(entity);
    ...
    public void beforeUpdate(AppCacheEntity entity) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "AppCacheEntity");
        }
    ...
    **/
    @Transactional
    public AppCacheEntity update(AppCacheEntity entity, List<String> updateFields) {
        beforeUpdate(entity);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<AppCacheEntity> batchUpdate(List<AppCacheEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<AppCacheEntity> data = ExcelUtil.read(file.getInputStream(), type, AppCacheEntity.class, entityFieldTitleNameMap);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public AppCacheEntity createOrUpdate(AppCacheEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, AppCacheEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            AppCacheEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                UserContext.UserInfo currentUserInfo = UserContext.getCurrentUserInfo();
                String currentUserName = null == currentUserInfo ? null : currentUserInfo.getUserName();
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(AppCacheEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
