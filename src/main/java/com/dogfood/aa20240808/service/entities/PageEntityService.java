...
import com.dogfood.aa20240808.repository.entities.PageEntityMapper;
...
import java.time.ZonedDateTime;
/**
* auto generate PageEntityService ServiceImpl
*
* @author sys
*/
@Service
public class PageEntityService extends AbstractService {
    @Resource
    private PageEntityMapper mapper;
    @Resource
    private RelationInnerService relationInnerService;
    @Resource
    private PageEntityService entityService;
    ...
    public PageEntityService() {
        entityFieldTitleNameMap.put("页面id", "id");
        ...
        entityFieldTitleNameMap.put("页面名称", "name");
        entityFiledColumnNameMap.put("name", "name");
        entityFieldTitleNameMap.put("关联模块id", "moduleId");
    ...
    **/
    public List<PageEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<PageEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, PageEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + PageEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
        ...
        public PageEntity get( Long id ) { 
            ...
            PageEntity entity = mapper.selectOne( id ); 
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<PageEntity> batchCreate(List<PageEntity> entities) {
        ...
        // fill default value
        List<PageEntity> batchList = new ArrayList<>(100);
    ...
    public void beforeUpdate(PageEntity entity) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "PageEntity");
        }
    ...
    **/
    @Transactional
    public PageEntity update(PageEntity entity, List<String> updateFields) {
        beforeUpdate(entity);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<PageEntity> batchUpdate(List<PageEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<PageEntity> data = ExcelUtil.read(file.getInputStream(), type, PageEntity.class, entityFieldTitleNameMap);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public PageEntity createOrUpdate(PageEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, PageEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            PageEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                UserContext.UserInfo currentUserInfo = UserContext.getCurrentUserInfo();
                String currentUserName = null == currentUserInfo ? null : currentUserInfo.getUserName();
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(PageEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
