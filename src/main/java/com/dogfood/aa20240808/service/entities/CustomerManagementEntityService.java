...
import com.dogfood.aa20240808.repository.entities.CustomerManagementEntityMapper;
...
import java.time.ZonedDateTime;
/**
* auto generate CustomerManagementEntityService ServiceImpl
*
* @author sys
*/
@Service
public class CustomerManagementEntityService extends AbstractService {
    @Resource
    private CustomerManagementEntityMapper mapper;
    @Resource
    private RelationInnerService relationInnerService;
    @Resource
    private CustomerManagementEntityService entityService;
    ...
    public CustomerManagementEntityService() {
        ...
        entityFieldTitleNameMap.put("客户编码", "customerCode");
        entityFiledColumnNameMap.put("customerCode", "customer_code");
        entityFieldTitleNameMap.put("客户名称", "customerName");
        entityFiledColumnNameMap.put("customerName", "customer_name");
        entityFieldTitleNameMap.put("联系人", "linkMan");
        entityFiledColumnNameMap.put("linkMan", "link_man");
        entityFieldTitleNameMap.put("联系电话", "contactNumber");
        entityFiledColumnNameMap.put("contactNumber", "contact_number");
        entityFieldTitleNameMap.put("联系地址", "address");
    ...
    **/
    public List<CustomerManagementEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<CustomerManagementEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, CustomerManagementEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + CustomerManagementEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
         ...
         * auto gen get method
         **/
        public CustomerManagementEntity get( Long id ) { 
            if ( id == null ) { 
                throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
            }

            CustomerManagementEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public CustomerManagementEntity create(CustomerManagementEntity entity) {
        if (null == entity) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<CustomerManagementEntity> batchCreate(List<CustomerManagementEntity> entities) {
        ...
        // fill default value
        List<CustomerManagementEntity> batchList = new ArrayList<>(100);
        for (CustomerManagementEntity entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            entity.setCreatedBy(StringUtils.isBlank(entity.getCreatedBy()) ? currentUserName : entity.getCreatedBy());
            entity.setUpdatedBy(StringUtils.isBlank(entity.getUpdatedBy()) ? currentUserName : entity.getUpdatedBy());
            paramValidate(entity);
    ...
    public void beforeUpdate(CustomerManagementEntity entity) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "CustomerManagementEntity");
        }
    ...
    **/
    @Transactional
    public CustomerManagementEntity update(CustomerManagementEntity entity, List<String> updateFields) {
        beforeUpdate(entity);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<CustomerManagementEntity> batchUpdate(List<CustomerManagementEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("CustomerManagement", "customer_management", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<CustomerManagementEntity>  entities = mapper.selectList(inQueryFilter);
        for (CustomerManagementEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<CustomerManagementEntity> data = ExcelUtil.read(file.getInputStream(), type, CustomerManagementEntity.class, entityFieldTitleNameMap);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public CustomerManagementEntity createOrUpdate(CustomerManagementEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, CustomerManagementEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            CustomerManagementEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                UserContext.UserInfo currentUserInfo = UserContext.getCurrentUserInfo();
                String currentUserName = null == currentUserInfo ? null : currentUserInfo.getUserName();
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(CustomerManagementEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
