...
import com.dogfood.aa20240808.repository.entities.FunctionTableEntityMapper;
...
import java.time.ZonedDateTime;
/**
* auto generate FunctionTableEntityService ServiceImpl
*
* @author sys
*/
@Service
public class FunctionTableEntityService extends AbstractService {
    @Resource
    private FunctionTableEntityMapper mapper;
    @Resource
    private RelationInnerService relationInnerService;
    @Resource
    private FunctionTableEntityService entityService;
    ...
    public FunctionTableEntityService() {
        entityFieldTitleNameMap.put("功能id", "id");
        ...
        entityFieldTitleNameMap.put("功能名称", "name");
        entityFiledColumnNameMap.put("name", "name");
        entityFieldTitleNameMap.put("功能描述", "description");
        entityFiledColumnNameMap.put("description", "description");
        entityFieldTitleNameMap.put("模块id", "moduleId");
        entityFiledColumnNameMap.put("moduleId", "module_id");
        entityFieldTitleNameMap.put("页面id", "pageId");
        entityFiledColumnNameMap.put("pageId", "page_id");
        entityFieldTitleNameMap.put("权限id", "permissionId");
    ...
    **/
    public List<FunctionTableEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<FunctionTableEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, FunctionTableEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + FunctionTableEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
         ...
         * auto gen get method
         **/
        public FunctionTableEntity get( Long id ) { 
            if ( id == null ) { 
                throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
            }

            FunctionTableEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public FunctionTableEntity create(FunctionTableEntity entity) {
        if (null == entity) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<FunctionTableEntity> batchCreate(List<FunctionTableEntity> entities) {
        ...
        // fill default value
        List<FunctionTableEntity> batchList = new ArrayList<>(100);
        for (FunctionTableEntity entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            entity.setCreatedBy(StringUtils.isBlank(entity.getCreatedBy()) ? currentUserName : entity.getCreatedBy());
            entity.setUpdatedBy(StringUtils.isBlank(entity.getUpdatedBy()) ? currentUserName : entity.getUpdatedBy());
            paramValidate(entity);
    ...
    public void beforeUpdate(FunctionTableEntity entity) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "FunctionTableEntity");
        }
    ...
    **/
    @Transactional
    public FunctionTableEntity update(FunctionTableEntity entity, List<String> updateFields) {
        beforeUpdate(entity);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<FunctionTableEntity> batchUpdate(List<FunctionTableEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("FunctionTable", "FunctionTable", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<FunctionTableEntity>  entities = mapper.selectList(inQueryFilter);
        for (FunctionTableEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<FunctionTableEntity> data = ExcelUtil.read(file.getInputStream(), type, FunctionTableEntity.class, entityFieldTitleNameMap);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public FunctionTableEntity createOrUpdate(FunctionTableEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, FunctionTableEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            FunctionTableEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                UserContext.UserInfo currentUserInfo = UserContext.getCurrentUserInfo();
                String currentUserName = null == currentUserInfo ? null : currentUserInfo.getUserName();
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(FunctionTableEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
