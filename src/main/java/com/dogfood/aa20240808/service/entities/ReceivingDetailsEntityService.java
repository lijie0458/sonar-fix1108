...
import java.math.BigDecimal;
/**
* auto generate ReceivingDetailsEntityService ServiceImpl
*
* @author sys
*/
@Service
public class ReceivingDetailsEntityService extends AbstractService {
    @Resource
    private ReceivingDetailsEntityMapper mapper;
    @Resource
    private RelationInnerService relationInnerService;
    @Resource
    private ReceivingDetailsEntityService entityService;
    ...
    public ReceivingDetailsEntityService() {
        entityFieldTitleNameMap.put("id", "id");
        entityFiledColumnNameMap.put("id", "id");
        entityFieldTitleNameMap.put("收货单号", "receiptNumber");
        entityFiledColumnNameMap.put("receiptNumber", "receipt_number");
        entityFieldTitleNameMap.put("本次收货数量", "receivedQuantity");
        entityFiledColumnNameMap.put("receivedQuantity", "received_quantity");
        entityFieldTitleNameMap.put("物料编码", "materialNumber");
        entityFiledColumnNameMap.put("materialNumber", "material_number");
        entityFieldTitleNameMap.put("关联明细Id", "orderDetailId");
        entityFiledColumnNameMap.put("orderDetailId", "order_detail_id");
    ...
    **/
    public List<ReceivingDetailsEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<ReceivingDetailsEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, ReceivingDetailsEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + ReceivingDetailsEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
         ...
         * auto gen get method
         **/
        public ReceivingDetailsEntity get( Long id ) { 
            if ( id == null ) { 
                throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
            }

            ReceivingDetailsEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public ReceivingDetailsEntity create(ReceivingDetailsEntity entity) {
        if (null == entity) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<ReceivingDetailsEntity> batchCreate(List<ReceivingDetailsEntity> entities) {
        ...
        // fill default value
        List<ReceivingDetailsEntity> batchList = new ArrayList<>(100);
        for (ReceivingDetailsEntity entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            paramValidate(entity);
    ...
    public void beforeUpdate(ReceivingDetailsEntity entity) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "ReceivingDetailsEntity");
        }
    ...
    **/
    @Transactional
    public ReceivingDetailsEntity update(ReceivingDetailsEntity entity, List<String> updateFields) {
        beforeUpdate(entity);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<ReceivingDetailsEntity> batchUpdate(List<ReceivingDetailsEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("ReceivingDetails", "receiving_details", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<ReceivingDetailsEntity>  entities = mapper.selectList(inQueryFilter);
        for (ReceivingDetailsEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<ReceivingDetailsEntity> data = ExcelUtil.read(file.getInputStream(), type, ReceivingDetailsEntity.class, entityFieldTitleNameMap);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public ReceivingDetailsEntity createOrUpdate(ReceivingDetailsEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, ReceivingDetailsEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            ReceivingDetailsEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                paramValidate(entity);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(ReceivingDetailsEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
