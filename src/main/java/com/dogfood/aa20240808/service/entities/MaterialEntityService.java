...
import com.dogfood.aa20240808.repository.entities.MaterialEntityMapper;
...
import java.time.ZonedDateTime;
/**
* auto generate MaterialEntityService ServiceImpl
*
* @author sys
*/
@Service
public class MaterialEntityService extends AbstractService {
    @Resource
    private MaterialEntityMapper mapper;
    @Resource
    private RelationInnerService relationInnerService;
    @Resource
    private MaterialEntityService entityService;
    ...
    public MaterialEntityService() {
        ...
        entityFieldTitleNameMap.put("物料分类", "category_name");
        entityFiledColumnNameMap.put("category_name", "category_name");
        entityFieldTitleNameMap.put("分类编码", "category_code");
        entityFiledColumnNameMap.put("category_code", "category_code");
        entityFieldTitleNameMap.put("上级分类", "parent_category_id");
        entityFiledColumnNameMap.put("parent_category_id", "parent_category_id");
    ...
    **/
    public List<MaterialEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<MaterialEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, MaterialEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + MaterialEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<MaterialEntity> batchCreate(List<MaterialEntity> entities) {
        ...
        // fill default value
        List<MaterialEntity> batchList = new ArrayList<>(100);
    ...
    public void beforeUpdate(MaterialEntity entity) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "MaterialEntity");
        }
    ...
    **/
    @Transactional
    public MaterialEntity update(MaterialEntity entity, List<String> updateFields) {
        beforeUpdate(entity);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<MaterialEntity> batchUpdate(List<MaterialEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<MaterialEntity> data = ExcelUtil.read(file.getInputStream(), type, MaterialEntity.class, entityFieldTitleNameMap);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public MaterialEntity createOrUpdate(MaterialEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, MaterialEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            MaterialEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                UserContext.UserInfo currentUserInfo = UserContext.getCurrentUserInfo();
                String currentUserName = null == currentUserInfo ? null : currentUserInfo.getUserName();
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(MaterialEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
